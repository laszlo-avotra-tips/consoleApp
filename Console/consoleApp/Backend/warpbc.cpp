...
WarpBc::WarpBc(cl_context context) :
...
bool WarpBc::enqueueCallKernelFunction()
{
    if(!m_signalModel){
        return false;
    }
...
bool WarpBc::initContext()
{
    bool success{false};
    success = createWarpBcImageBuffers(m_baseContext);
    return success;
}

void WarpBc::setKernel(cl_kernel kernel)
{
    if(!m_kernel){
        m_kernel = kernel;
...
bool WarpBc::setKernelArguments(cl_kernel kernel)
{
    ...
    clStatus |= clSetKernelArg( kernel, 13, sizeof(cl_float),    smi->blackLevel() );
    clStatus |= clSetKernelArg( kernel, 14, sizeof(cl_float),    smi->whiteLevel() );

    return  clStatus == CL_SUCCESS;
}

bool WarpBc::createWarpBcImageBuffers(cl_context context)
{
    cl_int err{-1};
