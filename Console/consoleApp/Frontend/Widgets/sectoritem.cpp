    ...
    else
    {
        ...
        if( angle_cnt != lastAngle_cnt )
        {
            ...
            if( !isUnwinding() )
            {
                 ...
                 */
                currentAngle_deg = ( (double)angle_cnt / EncoderCountsPerDeg ) - (double)getWindOffset();
...
//	qDebug() << ">>>>>> 10";
    ...
    float angle_rad     = currentAngle_deg * degToRad;
    float lastAngle_rad =    lastAngle_deg * degToRad;
    ...
    if( twoLinesValid )
    {
        ...
        */
        ...
        if( deltaTheta_rad > ( CrossOverAngleChange_rad ) )
        {
            ...
            {
                lastAngle_rad += 2.0 * pi;
            }
            deltaTheta_rad = ( 2.0 * pi ) - deltaTheta_rad;
            interpDirection *= -1;
        }

        // Interpolation: figure out the number of lines we need to fill in.
        int numberOfLinesToFill = (int)( deltaTheta_rad / MinInterpolationAngle_rad );
        ...
        float  correctedRho;
        int    skipCount;
        float  interpMultiplier = interpDirection * MinInterpolationAngle_rad;
