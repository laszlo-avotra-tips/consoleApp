...
//            bool success = connect( idaq->getSignalSource(), &IDAQ::updateSector, ms, &MainScreen::updateSector, Qt::QueuedConnection);
            bool success = connect( idaq, &IDAQ::updateSector, ms, &MainScreen::updateSector);
            LOG1(success)
            idaq->initDaq();
        }
//        idaq->start();
